      ...
      <div class="col-span-10 grid grid-cols-3 gap-4">
        <template v-for="item in discoverStore.state.discover.results">
          <Skeleton
            ...
            :loading="loading"
          >
            <figure class="border border-gray-200 mb-0 grid grid-cols-3 max-h-full">
              <Image
              ...
              />
              <figcaption
                class="p-2 col-span-2 text-sm grid grid-cols-max-1fr gap-x-2 gap-y-1 auto-rows-1fr break-all"
              >
                ...
                <template v-for="genre in item.genre_ids"
                  >{{ genreStore.findGenreNameById(genre) }},</template
                >
