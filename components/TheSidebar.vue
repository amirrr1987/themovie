...
const onTypeChange = async () => {
  await discoverStore.getDiscover({ type: discoverType.value, query: [] })
}
...
const searchHandler = async () => {
  const query: any = []
  queryStore.state.query.map((item: { label: any; value: any }) => {
    query.push(`&${item.label}=${item.value}`)
  })
  await discoverStore.getDiscover({ type: discoverType.value, query: query })
}
